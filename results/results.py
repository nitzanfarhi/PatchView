...
def save_accuracy(args, mall, run):
    ...
    if args.compare_languages:
        for language in mall['language'].unique():
            run.summary[f"best_threshold_{language}"], run.summary[f"best_accuracy_{language}"] = get_best_accuracy(mall[mall['language']==language])
            print(f"Final accuracy for {language}: ", run.summary[f"best_accuracy_{language}"])
            print(f"Best threshold for {language}: ", run.summary[f"best_threshold_{language}"])
...
def get_all_predictions(args, run):
    ...
    mall = pd.merge(pd.merge(code_df, message_df, left_index=True,
                right_index=True), event_df, left_index=True, right_index=True)

    mall['predicted_avg'] = mall[['Prediction_x',
...
def save_language_compare(mall, run):
    mall['language'] = mall.apply(lambda row: get_language_from_name(row['Name']), axis=1)
...
def example_explainability(mall, run, message_model_name):
    ...
    shap.plots.bar(shap_values[:,:,0].mean(0), show=False, max_display=20,  order=shap.Explanation.argsort.flip)
...
def parse_args():
    ...
    parser.add_argument("--code_artifact", type=str, default="nitzanfarhi/code4/run-ia1zinlc-test_table_0:v0",
                        help="Gather tables from wandb artifact")
    parser.add_argument("--message_artifact", type=str,
                        default="nitzanfarhi/message4/run-q8hktc1v-test_table_0:v0", help="Gather tables from wandb artifact")
    parser.add_argument("--event_artifact", type=str,
                        default="nitzanfarhi/events4/run-agwcpmq1-test_table_0:v0", help="Gather tables from wandb artifact")
    
    parser.add_argument("--message_model", type=str,
